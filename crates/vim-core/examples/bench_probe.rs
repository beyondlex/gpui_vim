...
impl VimBuffer for B {
    ...
    fn line_count(&self) -> usize { self.0.borrow().split('\n').count() }
    fn char_at(&self, o: usize) -> Option<char> { self.0.borrow()[o..].chars().next() }
    fn prev_char_offset(&self, o: usize) -> Option<usize> {
        if o == 0 { return None }
        self.0.borrow()[..o].chars().next_back().map(|c| o - c.len_utf8())
    }
    fn line_range(&self, line: usize) -> Range<usize> {
        let t = self.0.borrow();
        let mut s = 0;
        for (i, p) in t.split('\n').enumerate() {
            if i == line { return s..s + p.len() + usize::from(i + 1 < t.split('\n').count()) }
            s += p.len() + 1;
        }
        t.len()..t.len()
    }
    fn offset_to_line(&self, o: usize) -> usize { self.0.borrow()[..o.min(self.0.borrow().len())].split('\n').count() - 1 }
    fn slice(&self, r: Range<usize>) -> String { self.0.borrow()[r].to_owned() }
}
...
impl VimHost for H {
    fn viewport(&self) -> (usize, usize) { (0, 40) }
    fn scroll_to_line(&mut self, _: usize) {}
    fn clipboard_write(&mut self, _: &str) {}
    fn clipboard_read(&self) -> Option<String> { None }
    fn set_search_highlights(&mut self, m: &[Range<usize>], _: Option<Range<usize>>) { self.hl = m.to_vec() }
...
fn main() {
    ...
    println!("20x search re-scan (900KB, {} matches): {:>8.2?}  ({:.2} ms/scan)", matches, t.elapsed(), t.elapsed().as_millis() as f64 / 20.0);
    ...
    let mut vim = VimState::new();
    let mut h = H::default();
    let cmd: Vec<Key> = ":s/foo/bar/g".chars().map(Key::char).chain(std::iter::once(Key::named("enter"))).collect();
    let t = Instant::now();
